...
namespace CapaDatos
{
    public class DTurnos
    {
        ...
        #region METODO INSERTAR CIERRE TURNO
        public Task<string> InsertarCierreTurno(Turnos_cierres cierre)
        {
            string rpta = string.Empty;
            SqlConnection SqlCon = new SqlConnection(Conexion.Cn);
            try
            {
                ...
                {
                    Connection = SqlCon,
                    CommandText = "sp_Turnos_cierres_i",
                    CommandType = CommandType.StoredProcedure
                };

                SqlParameter Id_cierre = new SqlParameter()
                {
                    ParameterName = "@Id_cierre",
                    SqlDbType = SqlDbType.Int,
                    Direction = ParameterDirection.Output,
                };
                SqlCmd.Parameters.Add(Id_cierre);

                SqlParameter Id_turno = new SqlParameter()
                {
                    ParameterName = "@Id_turno",
                    SqlDbType = SqlDbType.Int,
                    Value = cierre.Id_turno,
                };
                SqlCmd.Parameters.Add(Id_turno);

                SqlParameter Fecha_cierre = new SqlParameter()
                {
                    ParameterName = "@Fecha_cierre",
                    SqlDbType = SqlDbType.Date,
                    Value = cierre.Fecha_cierre,
                };
                SqlCmd.Parameters.Add(Fecha_cierre);

                SqlParameter Hora_cierre = new SqlParameter()
                {
                    ParameterName = "@Hora_cierre",
                    SqlDbType = SqlDbType.Time,
                    Value = cierre.Hora_cierre,
                };
                SqlCmd.Parameters.Add(Hora_cierre);
                ...
                {
                    cierre.Id_cierre = Convert.ToInt32(SqlCmd.Parameters["@Id_cierre"].Value);
                }
            }
            catch (SqlException ex)
            {
        ...
        public Task<(string rpta, DataTable dtEstadistica, DataTable dtDetalle, DataTable dtPagos)> BuscarEstadistica(int id_turno)
        {
            string rpta = "OK";
            DataSet dsResultados = new DataSet();
            DataTable dtEstadistica = new DataTable("EstadisticaDiaria");
            ...
            try
            {
                ...
                {
                    dtEstadistica = dsResultados.Tables[0];
            ...
            return Task.FromResult((rpta, dtEstadistica, dtDetalles, dtPagos));
        }
    }
}
